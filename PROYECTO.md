...
- **`app/components/AuthIntro.tsx` (nuevo)**: hero ("Tu semestre, organizado." + una línea de bajada), las **5 secciones rotuladas de v10.11** (*Organizá tu semestre*, *Planificá cada día*, *Métricas reales* — con la card ocre del semestre montada sobre la esquina, igual que en PC —, *Tus apuntes, en audiolibros*, *Personalizá la app*) y una barra de acción abajo con **"Unirme a stuniv"** (→ `/registro`) y **"Ya tengo cuenta · Iniciar sesión"**. Una columna en el celular, **dos en tablet** (`sm:grid-cols-2`), donde una sola quedaría ridícula contra 700-1200px de ancho.
- **La barra de acción es `sticky bottom-0`**: entrar nunca queda a más de un toque, sin importar cuánto se scrollee el collage (era el pedido explícito de "que no haya que hacer mucho scroll para el log in"). Usa **el mismo material que el nav** (translúcido + blur + hairline) en vez de un degradé a canvas, que en modo **Vidrio** se habría visto como una banda clara contra el fondo del tema (#ECEBE5 + radiales) — de paso hace de contrapeso del header: la marca arriba, la acción abajo. Padding inferior con `env(safe-area-inset-bottom)` para el home indicator del iPhone.
- **`AuthCard.tsx`**: nueva prop `intro` (sólo la usa `/login`). El corte es **puramente por CSS** (`xl:hidden` / `hidden xl:flex`): en ≥1280px la card se muestra siempre, sin importar el estado — así **la versión de PC de v10.11 queda intacta** y no hay detección de viewport en JS ni riesgo de mismatch de hidratación. Con la portada activa aparece un "← Volver" arriba de la card (sólo si se llegó desde ella).
- **La portada NO se muestra cuando hay algo que leer/hacer ya mismo**: con `?error=confirmacion` (link vencido) va directo al form, y los links de vuelta desde `/registro` y `/recuperar` ahora apuntan a `/login?entrar=1` para no reabrirla. Verificado por SSR en las 5 rutas/variantes.
- **`app/components/authStyles.ts` (nuevo)**: `inputCls`/`labelCls`/`btnCls` se mudaron acá para que `AuthIntro` use el estilo del botón sin generar un import circular con `AuthCard` (que ahora importa `AuthIntro`). `AuthCard` los re-exporta → **las páginas no cambiaron ni un import**.
- **`Nav.tsx`**: en las pantallas de entrada el wordmark va **completo ("stuniv.") en todos los anchos**, no la "s." abreviada. La abreviatura existe porque en mobile el logo compite con la identidad académica y la hamburguesa — en `/login`, `/registro` y `/recuperar` la barra está vacía y la marca es lo único que se ve. (Segundo pedido de Jano.)

**Costo real en mobile**: la primera pintura suma **21KB** (sólo `inicio.webp`, la única captura sobre el pliegue — las otras cinco son `loading="lazy"`); el set completo son ~94KB y sólo se bajan si el usuario scrollea. **`/registro` y `/recuperar` en mobile siguen bajando CERO bytes de `/showcase/`** (verificado con `performance.getEntriesByType`). JS: **+1 kB gzip de First Load** en las tres rutas de entrada (login 136→137 kB, recuperar 136→137, registro 138→139; el chunk propio de cada página baja ~1.9 kB porque los componentes compartidos se hoistearon). Middleware **83.6 kB, sin cambios**.

**Verificación** (build en verde 28/28, medido en el navegador): **375×812** → portada con las 5 secciones, CTA visible en el viewport **sin scrollear** (700-744px de 812), card en `display:none`, y el árbol de accesibilidad expone sólo logo + 2 acciones (las capturas siguen `aria-hidden`). **768×1024** → dos columnas de 336px, CTA a 912px, apenas 253px de scroll total. **1279** → todavía portada. **1280 y 1920** → portada en `display:none`, vitrina en `grid`, card a 400px, y la **alineación de v10.11 intacta** (logo x=352 = vitrina x=352 en 1920). Estado post-tap verificado forzando el estado inicial: portada desmontada, "← Volver" arriba y **card en x=16 ancho 343 — idéntica a la de siempre**. En **Vidrio 3D**: CTA con el tinte navy translúcido y el gloss de la app, barra con el material del header, y las capturas **sin** adoptar el material (sin background, sin backdrop-filter, sin borde). Cero errores de consola.

*No verificado en vivo*: el tap real portada→form en el navegador de la sesión (el panel no compone frames, así que React nunca hidrata ahí y NADA de la app es interactivo en ese entorno). Se verificó el DOM de los dos estados por separado, como se describe arriba.
...
1. **Email ya registrado en el signup → mensaje CLARO (409)**: antes respondía "revisá tu casilla" aunque la cuenta ya existiera (anti-enumeración de Supabase) y parecía que "se pisaba" la cuenta. Ahora se detecta con la señal documentada de Supabase (`identities: []`) y responde "Ya existe una cuenta con ese email. Iniciá sesión o recuperala." **Verificado que la cuenta existente JAMÁS se pisa**: tras el 409 la contraseña original sigue funcionando (probado e2e). Trade-off de enumeración aceptado a propósito (CAPTCHA + 8 signups/h por IP; es lo que hacen las apps grandes). Si el email existe pero está SIN confirmar, el signup re-manda el mail de confirmación y muestra la pantalla de "Revisá tu casilla" (el flujo correcto para "me registré y no confirmé").
2. **Política de contraseñas real**: mínimo 8 con al menos una mayúscula, una minúscula y un número. Fuente única `app/lib/password.ts` compartida por server (signup, recover/verify, change-password) y cliente (registro, recuperar, con hint bajo el campo que se pone rojo si no cumple). ADEMÁS forzada a nivel Supabase Auth (`password_required_characters`) — ni un request a mano la esquiva. Las contraseñas viejas de cuentas existentes siguen sirviendo para entrar (la política aplica a contraseñas nuevas).
3. **Mails de confirmación llegando: VERIFICADO con mails reales** — 6 mails enviados en las pruebas a `soporte.stuniv+testN@gmail.com` (subdirecciones de la casilla de soporte real): 3 de signup, 2 reenvíos y 1 de recuperación. **Jano puede verlos en la bandeja de soporte.stuniv@gmail.com** (y de paso ver el template nuevo). SMTP: Gmail, 50 mails/h, mínimo 60s entre mails al mismo destinatario (lo fuerza Supabase).
4. **Reenvío del mail de confirmación** (como las apps grandes): endpoint nuevo `POST /api/auth/resend` (rate limit propio 3/h por email + 10/h por IP, fail-closed, respuesta genérica sin señal) + en la pantalla "Revisá tu casilla": botón con cooldown de 60s con cuenta regresiva, contador "Reenvíos usados: N de 3", y al agotar los 3 aparece el contacto `soporte.stuniv@gmail.com` (mailto con asunto y cuerpo pre-cargados). Turnstile propio en esa pantalla (token de un solo uso por reenvío). Capas verificadas e2e: reenvío inmediato → 429 (60s SMTP), reenvío tras 60s → 200 con mail real, 4º reenvío → 429 del límite propio (Retry-After ~20 min).
5. **Aviso de SPAM prominente**: caja destacada (borde+fondo ocre) en "Revisá tu casilla": "¿No aparece? Revisá la carpeta de SPAM / correo no deseado" + instrucción de marcar "No es spam". También en el paso del código de /recuperar.
6. **Mails brandeados**: templates de confirmación y recuperación rediseñados (Management API): card blanca con header navy y wordmark "stuniv.", botón navy "Confirmar mi cuenta" / código gigante espaciado, footer con soporte — HTML email-safe (tablas + estilos inline, sin imágenes externas). Subjects: "Confirmá tu cuenta · stuniv" / "Tu código para recuperar la cuenta · stuniv".
7. **La paleta ya NO se toca en el registro**: fuera la vista previa al elegir universidad y fuera el `tema_color` sugerido en el signup — todo arranca azul y se personaliza recién en /cuenta (que queda igual). Verificado: signup con UBA (que sugería negro) → perfil queda `azul`; el select de /registro no cambia ningún color (computed style idéntico).
8. **"Olvidé mi contraseña" FUNCIONA sin dominio propio** (usa el mismo SMTP de Gmail; verificado e2e con mail real de recuperación + la suite que cambia la contraseña con código real) — se queda.
9. **Revisión de variantes del flujo de entrada**: link de confirmación vencido/reusado → /login con mensaje claro nuevo ("si ya confirmaste iniciá sesión; si no, registrate de nuevo y te llega mail nuevo"); login sin confirmar → 403 con mención a SPAM y al reenvío; doble-submit protegido en todos los forms; tokens de captcha de un solo uso con reset tras fallo (también en el reenvío); anti-enumeración se CONSERVA en login/recuperar/reenviar (respuestas idénticas exista o no la cuenta); emails normalizados (trim+lowercase) en los dos lados; máximo 72 chars de contraseña (límite bcrypt).

**Archivos**: `app/lib/password.ts` (nuevo), `app/api/auth/resend/route.ts` (nuevo), signup/login/recover-verify/change-password routes, `app/lib/ratelimit.ts` (rlResend/rlResendIp), páginas registro/login/recuperar, suite Fase 2 (+3 checks). Config Supabase: `password_required_characters`, templates y subjects de mail (vía Management API; CAPTCHA se deshabilitó ~5 min para las pruebas e2e y quedó REACTIVADO y verificado).

**Verificación**: build en verde; suite Fase 2 **36/36 PASS** (incluye los checks nuevos de política y resend) y revocación **7/7 PASS** contra build de producción local; flujo completo probado en navegador real (form con hints, error de contraseña débil, pantalla "Revisá tu casilla" con caja de spam, cooldown 60s→0, reenvío OK con "Reenvíos usados: 1 de 3"); 409 + contraseña intacta + paleta azul verificados por API; usuarios de prueba borrados; CAPTCHA reactivado y verificado (token falso → 400).
...
1. **Backup con datos personales en repo PÚBLICO** (el repo `janoBordo/uca-economia` es público): `backups/uca_data-backup-2026-07-12.json` (agenda de exámenes y notas personales de Jano; sin credenciales) estaba trackeado. Sacado del repo (`git rm --cached`, el archivo local queda) + `backups/` al `.gitignore`. *Ojo: sigue en el historial de git* — si molesta, el paso opcional es `git filter-repo` + force push (destructivo, lo decide Jano; el contenido es de bajo riesgo).
2. **Worker de pdf.js servido por unpkg.com** (CDN de terceros ejecutando código en la app + `unpkg.com` entero permitido en la CSP): ahora es **self-hosted** — `scripts/copiar-pdf-worker.mjs` lo copia de `node_modules` a `public/` en `predev`/`prebuild` (siempre la MISMA versión que `pdfjs-dist`, requisito de la API; el archivo copiado está gitignoreado). `unpkg.com` **eliminado de la CSP** (`script-src` y `connect-src`) — cero CDNs de terceros sirviendo código ejecutable. El asset se excluyó del matcher del middleware (como `/logos`: público, sin gastar un `getUser` por fetch).
3. **Records de `POST /api/db` sin tope de claves** (`planEstudio`/`sesiones`/`preparacion`): un body malicioso podía traer miles de entradas (bloat en la base / un RPC por entrada en `_delta`). Ahora Zod corta en 400/50/50 claves (la UI real nunca pasa de ahí; cero cambio de comportamiento).
4. **`/api/tts` pasaba `tl` sin validar** a la URL upstream (siempre encodeado — no era inyectable — pero era input libre): ahora solo acepta forma de código de idioma (`^[a-z]{2,3}(-[A-Za-z]{2,4})?$`) o cae a `es`.
5. **Header `Cross-Origin-Opener-Policy: same-origin`** agregado (aísla la ventana de referencias cross-origin; no afecta Turnstile ni Web Share).

**Verificado SIN hallazgos (lo importante del checklist):** RLS habilitado+forzado con políticas por operación en las 6 tablas y enforcement de soft-delete (0001/0002); IDOR probado por suite (upsert/`_delta`/aislación A/B); sesión SOLO en cookies HttpOnly+Secure+SameSite=Lax (nada en localStorage: solo tema/paleta/estado del timer); logout/eliminación revocan server-side (suite 7/7); anti-enumeración en login/signup/recover; OTP con rate limit fail-closed; **cero secretos en el código NI en los 88 commits del historial** (escaneo de patrones sb_secret/JWT/SG./AKIA/ghp_ sobre `git rev-list --all`; las únicas menciones son texto de docs); `.env*` nunca commiteado; solo `NEXT_PUBLIC_*` públicas llegan al cliente; **CORS: las APIs no emiten ningún `Access-Control-*`** (verificado contra producción con Origin cruzado y preflight — el `Access-Control-Allow-Origin: *` que se ve en `/login` lo agrega el CDN de Vercel SOLO en páginas estáticas públicas cacheadas, sin credenciales: inofensivo); headers de seguridad completos en prod; source maps de producción off; sin páginas de debug (`/configuracion` es un redirect); errores genéricos al cliente + detalle en logs (sin tokens/contraseñas logueados); XSS: React escapa todo, los 2 `dangerouslySetInnerHTML` son strings estáticos propios (anti-flash y GA init), sin HTML de usuario renderizado; SQLi: todo por PostgREST/RPCs con inputs validados por Zod (los uuid interpolados en filtros ya pasaron por `z.string().uuid()`); uploads: magic bytes + 400KB + bucket privado + path derivado de la sesión; SSRF: ningún fetch server-side a URL de usuario; **backups: verificado que el job diario `backup-db` del repo privado `stuniv-backups` viene corriendo en verde (última corrida hoy)**; `pdfjs-dist` 4.4.168 libre del CVE-2024-4367 (ejecución de JS vía PDF malicioso, fixed en 4.2.67 — importante porque los usuarios cargan PDFs de terceros en Lectura).

**Pendientes que decide Jano (no se resuelven solos):**
1. **Migración `0004_sesion_viva_solo_propia.sql` (nueva, SIN aplicar)**: hardening de mínimo privilegio — `sesion_viva()` (SECURITY DEFINER) hoy responde si CUALQUIER uuid de sesión existe; el cambio la limita a sesiones del propio usuario. Riesgo actual bajísimo (uuid v4 no adivinable) — por la regla fija de "migraciones nunca automáticas en producción" queda para aplicar A MANO desde el SQL Editor y re-correr `test-revocacion-e2e.mjs` (debe dar 7/7).
2. **Next 14.2.35 → 16 (breaking)**: los advisories de npm audit siguen (1 high + 1 moderate, el fix es Next 16). De los CVEs listados, la mayoría NO aplica al setup (sin rewrites, sin next/image, sin nonces, sin i18n Pages Router, hosting Vercel no self-hosted); aplican parcialmente los DoS/cache-poisoning de Server Components. Recomendación: planificar el upgrade como una versión propia (trae React 19), no urgente pero sí en el roadmap.
3. **Tokens de gestión amplios todavía en `.env.local`** (SUPABASE_ACCESS_TOKEN, VERCEL_TOKEN, UPSTASH_API_KEY, SENDGRID, SENTRY, POSTHOG, SUPABASE_DB_PASSWORD): es el paso 6 pendiente del cierre de v10 — no están filtrados (archivo local, fuera de git), pero la regla de credenciales angostas dice revocar/limpiar los que no se usan. Ojo: las suites e2e usan SUPABASE_SECRET_KEY (queda) y el toggle de CAPTCHA de la suite de Fase 1 usa SUPABASE_ACCESS_TOKEN.
...
**Verificación**: build en verde; suite **nueva** `scripts/test-revocacion-e2e.mjs` 7/7 PASS (logout con cookies → `/api/db` 401 INMEDIATO con token aún válido; eliminar cuenta vía Bearer → 401 INMEDIATO); suite Fase 3 re-corrida **33/33 PASS** (el RPC replica el contrato del getData viejo exacto: orden de materias, formato de examen, `?full=1`, archivar, aislación A/B). **En producción (commit `00c0885`)**: la suite de revocación corrida CONTRA el deploy vivo (`stuniv.vercel.app`) — **7/7 PASS**, incluyendo GET con sesión real → 200 vía el RPC y ambos "→ 401 INMEDIATO" con el token aún criptográficamente válido.
...
### v10.12.2 — La compu vuelve a ser una sola ventana + botón de compartir
- **Corte de PC: xl (1280px) → lg (1024px)**. La portada de entrada de v10.12 es para celular, pero el corte estaba en 1280px y una laptop con el escalado de Windows (125–175%) reporta un viewport menor a eso → en compu aparecía la portada apilada en vez de la ventana única. Ahora desde 1024px vuelve el layout de siempre: vitrina de capturas a la izquierda, card de login/registro a la derecha, todo en una sola pantalla. Sigue siendo puro CSS (`lg:hidden` / `hidden lg:flex`), sin detección de viewport en JS.
  - Para que entre cómodo en 1024–1280px se ajustaron sólo espaciados: card `22rem` (vuelve a `25rem` desde xl), gap 8 (12 desde xl), gap-x de la vitrina 6 (8 desde xl) y la columna derecha con `mt-16`/`space-y-9` (24/12 desde xl). De 1280px para arriba **nada cambió**.
...
### v10.14 — Que se sienta rápida con muchos usuarios (sin bajar la guardia de seguridad)
Pasada completa de performance. Nada de lo que se hizo relaja un control de seguridad: RLS sigue forzado, las cookies siguen HttpOnly, los límites fail-closed siguen fail-closed y ninguna clave nueva sale del server. Lo que cambió es **cuándo** se piden las cosas, **cuánto** se descarga y **cuántas veces** se le pega a la base.

**1. La espera de red ya no va detrás de la del JavaScript.**
Antes la secuencia era: HTML → descargar el bundle → hidratar React → *recién ahí* pedir `/api/db` y el perfil. Dos esperas encadenadas. Ahora un script inline en el `<head>` de `layout.tsx` larga esos dos pedidos apenas el navegador parsea el HTML, o sea **en paralelo** con la descarga del JS; `app/lib/api.ts` y `app/lib/perfil.ts` adoptan esa respuesta ya en vuelo (`window.__stunivPre`) en vez de pedir lo mismo de nuevo. Son los mismos requests de siempre —mismo endpoint, mismas cookies, mismo 401— sólo que largados antes. No se disparan en `/login`, `/registro`, `/recuperar` ni `/auth/*` (no hay sesión que pedir), y en `/semestre` sale directo con `?full=1`. Verificado en el navegador: en las pantallas públicas `window.__stunivPre` no existe.

**2. Cache persistente del cliente + esqueleto, sin romper la hidratación.**
El cache en memoria se borraba en cada carga completa de página (F5, abrir la app de cero), así que la primera pintura era siempre vacía. Ahora el último `AppData` y el perfil se espejan en `localStorage` y se rehidratan antes del primer render (`stuniv_db_v1` / `stuniv_perfil_v1`), con revalidación en segundo plano. **Importante**: ese estado NO se usa como valor inicial de `useState` —eso rompía la hidratación de React contra el HTML estático y costaba un re-render del árbol entero, justo lo contrario de lo buscado—; se adopta en un layout effect (`app/lib/hidratacion.ts`), que corre antes del paint. El HTML estático de `/` trae un esqueleto del hero con las mismas alturas que el contenido real, así que la primera pintura llega sin esperar nada.
- **Multiusuario**: el espejo se borra al entrar, al salir, al borrar la cuenta y ante cualquier 401. `/auth/confirm` redirige a `/?bienvenida=1` y esa marca tira el espejo del usuario anterior *antes* de pintar. En una compu compartida, la sesión siguiente nunca puede ver datos de la anterior.
- **`fotoUrl` no se espeja** (es una URL firmada que caduca): se pintan las iniciales y la foto entra con la revalidación.

**3. El perfil dejó de re-pedirse en cada pantalla… y volvió a revalidar.**
`fetchPerfil()` cortaba con `if (cache) return cache`, y con el espejo persistente eso significaba **no revalidar nunca**: el perfil rehidratado quedaba congelado y la foto no volvía a aparecer jamás. Ahora hay TTL de 5 minutos con marca de tiempo propia, arrancando vencido: la primera carga de cada pestaña siempre confirma contra el server y las navegaciones siguientes salen del cache. Ante un fallo (401, red, 429) se conserva lo que ya había en vez de vaciar el Nav.

**4. Menos round-trips por request en el server.**
- `/api/account/profile` GET corría `getUser()` y *después* la lectura de la fila: dos viajes a Supabase encadenados, en cada carga de página (el Nav muestra nombre y avatar). Ahora van **en paralelo**. Se puede porque la política `profiles_select_own` ya limita el `select` a la fila propia y activa — el `.eq("id", …)` era redundante con la RLS, no la protección; si no hay sesión válida, `getUser` falla y se corta con 401 igual que antes.
- Límite de datos **por usuario** en vez de por IP (con una reja previa por IP de techo alto, sólo anti-inundación). Detrás del NAT del wifi de una facultad todos los alumnos comparten una IP: con el límite viejo se pisaban entre ellos y la app les tiraba 429 sin que nadie hiciera nada raro. Aplica a `/api/db` (GET y POST), `/api/auth/me` y `/api/account/profile`.
- `middleware.ts` verifica la **firma del JWT en local** (ES256 + JWKS cacheado) en vez de pagar un `getUser()` contra Supabase en cada navegación — era el piso de latencia de toda la app y crecía con la cantidad de usuarios. Cae a `getUser()` cuando el token venció, que es justo cuando hay que refrescar la sesión. La revocación la sigue cortando `/api/db` en el mismo round-trip de datos (`get_app_data` → `sesion_revocada` → 401 → a `/login`): el middleware nunca fue la línea real de protección de datos.
- **Los limiters en memoria salieron a `app/lib/ratelimit-local.ts`.** `ratelimit.ts` importa el SDK de Upstash en el tope, así que *cualquier* ruta que pidiera un limiter arrastraba **105 kB** de Redis a su bundle serverless aunque no lo usara. `/api/db` —la ruta más caliente— y `/api/auth/me` sólo usan limiters locales: ahora no lo cargan más (verificado en las trazas del build). Las rutas de seguridad siguen con Redis, compartido entre instancias y fail-closed: eso no se tocó.

**5. Los sliders de Métricas guardaban en cada píxel del arrastre.**
`onChange` de un `input[type=range]` dispara decenas de veces por arrastre, y cada una mandaba un POST a `/api/db` que valida sesión contra el Auth server, escribe una fila por valor distinto y vuelve a leer todo el estado. Con un par de arrastres se llegaba al techo de 120 pedidos/minuto y empezaban los 429 — la pantalla que más escribe era la que primero se rompía con varios usuarios. Ahora el número se mueve al instante (state local, igual que antes) y el guardado sale **una sola vez**, 500 ms después del último movimiento; al salir de la pantalla se descarga lo pendiente.

**6. Bundle: −29 kB comprimidos en TODAS las páginas (~21%).**
`framer-motion` se usaba con `motion.*`, que arrastra la librería entera. La app anima opacidad, desplazamientos, escalas, hover/tap y entradas/salidas — exactamente el subconjunto `domAnimation`. Se pasó a `LazyMotion` + `m` (`app/components/MotionProvider.tsx` envuelve toda la app desde el layout raíz) con `import { m as motion }`, así el JSX de las 13 vistas quedó **idéntico**. Lo único que quedaba fuera de `domAnimation` eran las animaciones de layout, usadas en una sola cosa: la pastilla que se desliza entre las pestañas del Nav — ahora se mide la pestaña activa y se mueve un solo elemento con una transición CSS (con `ResizeObserver` para reacomodarla cuando entra la tipografía Inter). Mismo deslizamiento, cero JS de animación.
- First Load JS: `/` 136 → **107 kB**, `/login` 140 → **111 kB**, `/calendario` `/semestre` `/timer` 131 → **103 kB**, `/tts` 130 → **101 kB**, `/cuenta` 134 → **106 kB**, `/metricas` 128 → **100 kB**.

**7. Google Analytics dejó de competir con la app.**
`gtag.js` son ~100 kB de JavaScript de terceros y con `afterInteractive` cargaban justo mientras React hidrataba: el usuario esperaba a la analítica para poder tocar la pantalla. Pasó a `lazyOnload` (arranca después del load). `track()` ya era fire-and-forget, así que nada se rompe si gtag todavía no llegó.

**8. Animaciones de entrada más cortas (el contenido ya estaba; lo tapaba la animación).**
Transición de página de 0.5s/20px → **0.28s/8px**. En Inicio, el escalonado de las materias estaba en `0.4 + i*0.05`: con 9 materias la última fila aparecía recién a los 0.85s. Ahora corta a 0.4s en total, y lo mismo en los sliders de Métricas (tope 0.3s).

**Qué NO se hizo, y por qué**
- **Pasar `/api/db` a edge runtime**: parece un win por el cold start, pero edge corre en la ubicación más cercana al usuario, que puede quedar *más lejos* de la base — en una ruta que hace un round-trip a Postgres suele ser peor. Queda en nodejs, cerca de Supabase.
- **Sacar `framer-motion` del todo** (reescribir las animaciones en CSS): sería el máximo ahorro, pero son 13 archivos de comportamiento visual con riesgo real de regresión. `LazyMotion` da la mayor parte del beneficio sin tocar una sola animación.
- **`prefetch={false}` en el Nav**: ahorraría invocaciones de middleware, pero es exactamente lo que hace que la navegación se sienta instantánea. Se deja.
- **Espejar `fotoUrl`**: es una URL firmada; guardarla en `localStorage` para ahorrar un parpadeo no vale el cambio de postura.

**Verificación**: `tsc --noEmit` y `next build` limpios; tamaños de bundle medidos contra el build anterior; trazas del build confirmando que `/api/db` ya no incluye Upstash; `/login` y `/registro` abiertos en el navegador con el build de producción (contenido visible ⇒ `LazyMotion` bien cableado, si no las vistas quedarían en `opacity:0`) y la lógica del script del `<head>` probada ruta por ruta. **Lo que no pude mirar**: las pantallas con sesión (Inicio, Timer, Métricas, Calendario, Semestres, Cuenta) — exigen login y no tengo credenciales de prueba. Conviene una pasada a ojo por esas seis, sobre todo por la pastilla del Nav y el esqueleto de Inicio.
